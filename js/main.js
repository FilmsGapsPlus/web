document.addEventListener("DOMContentLoaded", () => {
    ...
    heroSection.innerHTML = `
      ...
      <div class="hero-content fadeInUp">
        ...
        <div class="hero-buttons">
          <button class="btn btn-red" onclick="window.openContentModal('${type}', ${type === "movies" ? allMovies.indexOf(content) : allSeries.indexOf(content)})">
  ...
  function createGenreSection(genreTitle, content, icon = "fas fa-film", type) {
    ...
    content.slice(0, 20).forEach((item, index) => {
      ...
      card.addEventListener("click", () => {
        ...
        window.openContentModal(contentType, realIndex)
      })
  ...
  window.openContentModal = (type, contentIndex) => {
    const content = isSearchActive
      ? type === "movies"
        ? searchResultsMovies[contentIndex]
        : searchResultsSeries[contentIndex]
      : type === "movies"
        ? allMovies[contentIndex]
        : allSeries[contentIndex]
    if (!content) return
    ...
    allGenres.forEach((genre) => {
      ...
      genreCard.addEventListener("click", () => {
        ...
        if (currentContentType === "movies") {
          ...
          filteredContent = [...moviesFiltered, ...seriesFiltered]
        }
        ...
        if (filteredContent.length > 0) {
          createGenreSection(
            ...
            filteredContent,
            icon,
            currentContentType === "channels" ? "mixed" : currentContentType,
          )
