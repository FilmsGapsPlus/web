document.addEventListener('DOMContentLoaded', function() {
	...
	async function loadContent(type) {
		...
		try {
			...
			if (data.success && data.data.length > 0) {
				...
				contentContainer.innerHTML = `<p>No se encontró ${type === 'movies' ? 'películas' : 'series'}</p>`;
			}
		} catch (error) {
			console.error(`Error al cargar ${type === 'movies' ? 'películas' : 'series'}:`, error);
			contentContainer.innerHTML = `<p>Error al cargar ${type === 'movies' ? 'las películas' : 'las series'}. Por favor, intenta de nuevo más tarde.</p>`;
		} finally {
			isLoading = false;
	...
	function organizeContentByGenre(type) {
		...
		allGenres.forEach(genre => {
			contentByGenre[genre] = contentArray.filter(item =>
				item.generos && item.generos.includes(genre)
			);
		});
	...
	function loadGenresModal() {
		...
		allGenres.forEach(genre => {
			...
			genreCard.addEventListener('click', () => {
				...
				const contentArray = currentContentType === 'movies' ? allMovies : allSeries;
				...
				if (filteredContent.length > 0) {
					createGenreSection(`${genre}`, filteredContent, icon, currentContentType);
				} else {
					contentContainer.innerHTML = `
                                <div class="fadeInUp" style="text-align: center; padding: 3rem 1rem;">
                                    ...
                                    <button class="btn btn-primary" onclick="loadContent('${currentContentType}')">
                                        <i class="fas fa-home"></i> Volver al inicio
                                    </button>
                                </div>
                            `;
